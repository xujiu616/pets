...
async function init() {
  ...
  try {
    ...
    const response = await fetch('data/file_index.json');
    ...
    let errorDetails = `
      ...
      2. 检查 JSON 格式是否正确<br>
      3. 确认通过 http://localhost:8000 访问<br>
      4. 查看浏览器控制台获取更多信息
    `;
...
async function loadPet(petName) {
  ...
  // 尝试多种可能的图片文件名格式
  const imageSources = [
    `images/pets/${petName}.png${cacheBuster}`,
    `images/pets/${petName}.jpg${cacheBuster}`,
    `images/pets/${petName}.jpeg${cacheBuster}`,
    `images/pets/${petName}_no_bg.png${cacheBuster}`
  ];
  ...
  try {
    ...
    const filenameVariants = [
      `data/${petName}技能体系.txt${cacheBuster}`,
      `data/${petName} 技能体系.txt${cacheBuster}`,
      `data/${petName}_技能体系.txt${cacheBuster}`
    ];
...
function showAltText(petName) {
  ...
  altText.onclick = function() {
    showImageModal(`images/pets/${petName}.png`, `${petName} (图片未找到)`);
  };
...
function createSkillCard(skillName, desc, petName) {
  ...
  // 使用宠物特定的技能图片路径
  const skillImagePath = `images/skills/${petName}/${cleanSkillName}.png`;
